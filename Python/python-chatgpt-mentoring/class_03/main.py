from class_03.types import BillPayload, ValidationResult
from class_03.utils.to_float import to_float
from class_03.data.constants import ERRORS, ALLOWED_CURRENCIES, ALLOWED_PAYMENT_METHODS, ALLOWED_STATUSES
from class_03.data.test_payloads import valid_payload, invalid_currency_payload, invalid_payment_method_payload, empty_payment_method_payload, extra_field_payload
...
def validate_bill_payload(payload: BillPayload) -> ValidationResult:
    ...
    if not normalized_payment_method:
        return False, ERRORS["err_payment_method_empty"]

    if normalized_payment_method not in ALLOWED_PAYMENT_METHODS:
        return False, ERRORS["err_payment_method_invalid"]
